...
######## Movement handler for rats in the environment ########
class RatMovementHandler:
    """
    Handles movement for rats. Rats support a few different movement styles,
    including a "wall_random" style that keeps rats confined to the
    perimeter (walls/corners) of the grid, randomly walking clockwise or
    counter-clockwise (or staying put) along that perimeter each step.
    """
    def __init__(self, grid_size: int, rat_movement_type: str = "wall_random", rounding_digits: int = 2):
        """
        Args:
            grid_size: Size of the environment grid
            movement_type: One of ["stationary", "discrete_random", "continuous_random", "wall_random"]
            rounding_digits: Number of digits to round position coordinates to
        """
        self.grid_size = grid_size
        self.movement_type = rat_movement_type
        self.rounding_digits = rounding_digits
        self.momentum = 0.8
        self.max_acceleration = 0.2
        self.max_velocity = 1.0
        ...
        # Precompute the ordered perimeter cycle (wall + corners) used by wall_random
    ...
    def _build_perimeter_cycle(self) -> List[Tuple[int, int]]:
        """
        Build an ordered, cyclical list of the integer grid cells that make up
        the outer wall (including all four corners) of the grid, going
        clockwise starting from (0, 0). Walking +1/-1 along this list is
        equivalent to walking along the walls and turning corners naturally.
        """
        n = self.grid_size
        cycle = []
        # Left wall, bottom -> top (x = 0)
        for y in range(0, n):
            cycle.append((0, y))
        # Top wall, left -> right (y = n-1)
        for x in range(1, n):
            cycle.append((x, n - 1))
        # Right wall, top -> bottom (x = n-1)
        for y in range(n - 2, -1, -1):
            cycle.append((n - 1, y))
        # Bottom wall, right -> left (y = 0), excluding the corners already added
        for x in range(n - 2, 0, -1):
            cycle.append((x, 0))
        return cycle

    def initialize_positions(self, n_rats: int, rng: np.random.Generator) -> list:
        """
        Initialize starting positions for all rats based on movement type.
        Returns a list of (x, y) tuples.
        """
        positions = []
        if self.movement_type == "wall_random":
            cycle_len = len(self._perimeter_cycle)
            for i in range(n_rats):
                idx = int(rng.integers(0, cycle_len))
                self.perimeter_indices[i] = idx
                positions.append(self._perimeter_cycle[idx])
        else:
            for i in range(n_rats):
                x = round(rng.uniform(0, self.grid_size), self.rounding_digits)
                y = round(rng.uniform(0, self.grid_size), self.rounding_digits)
                positions.append((x, y))
                if self.movement_type == "continuous_random":
                    vx = rng.uniform(-self.max_velocity, self.max_velocity)
                    vy = rng.uniform(-self.max_velocity, self.max_velocity)
    ...
    def _continuous_random_move(self, x: float, y: float, rat_id: int, rng: np.random.Generator) -> Tuple[float, float]:
        ...
        if rat_id not in self.velocities:
            vx = rng.uniform(-self.max_velocity, self.max_velocity)
            vy = rng.uniform(-self.max_velocity, self.max_velocity)
            self.velocities[rat_id] = [vx, vy]

        vx, vy = self.velocities[rat_id]
        sigma = 1.0 / 3.0
        rand_x = np.clip(rng.normal(loc=0.0, scale=sigma), -1.0, 1.0)
        rand_y = np.clip(rng.normal(loc=0.0, scale=sigma), -1.0, 1.0)
        ax = rand_x * self.max_acceleration
        ay = rand_y * self.max_acceleration
    ...
    def _wall_random_move(self, rat_id: int, rng: np.random.Generator) -> Tuple[int, int]:
        """
        Random movement restricted to the perimeter (walls and corners) of the
        grid. The rat takes a random step of -1, 0, or +1 along the ordered
        perimeter cycle, which naturally makes it hug the walls and turn
        corners as it moves.
        """
        cycle_len = len(self._perimeter_cycle)
        current_idx = self.perimeter_indices.get(rat_id, 0)
...
class Excreta:
    """
    A single pile of rat excreta left behind on the grid. It sits at a fixed
    position and disappears (expires) after a configurable number of steps.
    """
    _next_id = 1

    def __init__(self, x: float, y: float, lifespan: int, id: int = None):
        """
        Args:
            x, y: position where the excreta was left
            lifespan: number of steps the excreta persists before disappearing
            id: optional manual ID assignment
        """
        if id is None:
            self.id = Excreta._next_id
            Excreta._next_id += 1
        else:
            self.id = id
        ...
        self.age = 0  # steps elapsed since it was left

    def step(self) -> bool:
        """
        Age the excreta by one step.
        Returns True if the excreta has expired (should be removed).
        """
        self.age += 1
...
class Rat:
    """
    A rat that roams the grid and periodically leaves excreta behind.
    Movement style is delegated to a RatMovementHandler, so a rat can move
    randomly anywhere on the grid, stay put, move smoothly, or - notably -
    randomly wander along the walls/corners of the grid (wall_random).
    """
    _next_id = 1

    def __init__(self, x: float, y: float, id: int = None, excretion_prob: float = 0.1):
        """
        Args:
            x, y: initial position
            id: optional manual ID assignment
            excretion_prob: probability that the rat leaves a pile of excreta
                at its current position on any given step
        """
        if id is None:
            self.id = Rat._next_id
            Rat._next_id += 1
        else:
            self.id = id
    ...
    def maybe_leave_excreta(self, rng: np.random.Generator, lifespan: int) -> Optional["Excreta"]:
        """
        Roll the dice on whether this rat leaves excreta at its current
        position this step. Returns a new Excreta instance, or None.
        """
        if rng.random() < self.excretion_prob:
            return Excreta(self.x, self.y, lifespan)
        return None
...
class SIRSDEnvironment(gym.Env):
    ...
    def __init__(
        ...
        ##############################
        self.n_rats = n_rats
        self.rat_excretion_prob = rat_excretion_prob
        self.excreta_lifespan = excreta_lifespan
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[dict, dict]:
        ...
        self.excreta = []
        if self.n_rats > 0:
            rat_positions = self.rat_movement_handler.initialize_positions(self.n_rats, self.np_random)
            for i, (x, y) in enumerate(rat_positions):
                self.rats.append(Rat(x, y, id=i + 1, excretion_prob=self.rat_excretion_prob))
    ...
    def _handle_rat_stepping(self):
        """
        Move each rat according to the configured rat movement type, let rats
        probabilistically leave excreta behind, and age/expire existing
        excreta piles.
        """
        ...
        for rat in self.rats:
            ...
            if new_excreta is not None:
                self.excreta.append(new_excreta)

        # Age existing excreta and drop any that have expired
        if self.excreta:
            self.excreta = [e for e in self.excreta if not e.step()]
    ...
    def _render_frame(self) -> Optional[np.ndarray]:
        ...
        if self.excreta:
            ...
            ax_grid.scatter(ex_x, ex_y,
                          c=self.COLORS['excreta'],
                          s=30,
                          alpha=0.85,
                          marker='s',  # square marker to visually distinguish from agents
                          label='Excreta',
                          edgecolors='none',
                          zorder=1)
        ...
        # Plot rats
        if self.rats:
            rat_x = [r.x for r in self.rats]
            rat_y = [r.y for r in self.rats]
            ax_grid.scatter(rat_x, rat_y,
                          c=self.COLORS['rat'],
                          s=60,
                          alpha=0.9,
                          marker='^',  # triangle marker to distinguish rats from humans
                          label='Rats',
                          edgecolors='white',
                          linewidth=1,
                          zorder=3)
